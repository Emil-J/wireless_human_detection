...
uint8_t command_strobe(uint8_t addr)
{
	...
	SPI_CS_WRITE(0);
	while(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_2));
	HAL_SPI_TransmitReceive(&hspi2, &addr, &status, 1, 0xFFFF);
	SPI_CS_WRITE(1);
...
uint8_t read_status_reg(uint8_t addr, uint8_t *statusByte)
{
	...
	if(addr >= 0x30 && addr <= 0x3D)
	{
		SPI_CS_WRITE(0);
		while(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_2));
		HAL_SPI_TransmitReceive(&hspi2, &statusReg, statusByte, 1, 0xFFFF);
		delay_25ns(4);		//delay od 100ns
		HAL_SPI_TransmitReceive(&hspi2, &txDummy, &statusRegValue, 1, 0xFFFF);
		SPI_CS_WRITE(1);
...
uint8_t receive_burst_byte_433(uint8_t *rxData)
{
	...
	if(num_rxBytes == 5)
	{
		read_burst_byte(RX_FIFO, &statusByte, rxData, num_rxBytes);
...
uint8_t read_single_byte(uint8_t addr, uint8_t *statusByte)
{
	...
	SPI_CS_WRITE(0);
	while(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_2));
	HAL_SPI_TransmitReceive(&hspi2, &txData, statusByte, 1, 0xFFFF);
	delay_25ns(4);					//delay od 100ns
	HAL_SPI_TransmitReceive(&hspi2, &txDummy, &dataRx, 1, 0xFFFF);
	SPI_CS_WRITE(1);
...
void read_burst_byte(uint8_t startAddr, uint8_t *statusByte, uint8_t *rxData, uint8_t len)
{
	...
	if(startAddr <= 0x2F || startAddr == 0x3E || startAddr == 0x3F)
	{
		...
		SPI_CS_WRITE(0);
		while(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_2));
		HAL_SPI_TransmitReceive(&hspi2, &addr, statusByte, 1, 0xFFFF);
		delay_25ns(4);				//delay od 100ns
		if(len>64)
		{
			len=64;
		...
		for(int i=0; i < len; i++)
		{
			HAL_SPI_TransmitReceive(&hspi2, &txDummy, &rxData[i], 1, 0xFFFF);
			if(i<(len-1))
			{
...
void write_burst_byte(uint8_t startAddr, uint8_t *statusByte, uint8_t *txData, uint8_t len)
{
	if(startAddr <= 0x2F || startAddr == 0x3E || startAddr == 0x3F)
	{
		...
		for(int i=0; i < len; i++)
		{
			HAL_SPI_TransmitReceive(&hspi2, &txData[i], statusByte, 1, 0xFFFF);
			if(i<(len-1))
			{
				delay_25ns(4);		//delay od 100ns, cc1101 datasheet pg.30
			}
